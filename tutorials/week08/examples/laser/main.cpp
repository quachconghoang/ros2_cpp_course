    ...
    std::this_thread::sleep_for (std::chrono::seconds(1)); //Small delay befroe starting the loop
     ...
     for (unsigned int i=0;i<repeats;i++){
         ...
         if(OK){
            ...
            auto it = std::min_element(std::begin(laserScan.ranges), std::end(laserScan.ranges));
            auto index = std::distance(std::begin(laserScan.ranges), it);
            double angle = laserScan.angle_min + index * laserScan.angle_increment;
            std::cout << "Closest point: " << *it << " at angle: " << angle << std::endl;
            double x = *it * cos(angle);
            double y = *it * sin(angle);
